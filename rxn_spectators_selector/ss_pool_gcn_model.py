# -*- coding: utf-8 -*-
# @Time     : 2021/5/18 15:54
# @Author   : zbc
# @Email    : zhangbc0315@outlook.com
# @FileName : ss_pool_gcn_model.py
# @Software : PyCharm
...
class SSPoolGcnModel(Module):
    ...
    def __init__(self, num_node_features, num_edge_features, num_g, out_channels):
        super(SSPoolGcnModel, self).__init__()
        self.out_channels = out_channels
        self.meta_r1 = gnn.MetaLayer(EdgeModel(num_node_features, num_edge_features, 512),
                                     NodeModel(num_node_features, 512, 128),
                                     GlobalModel(128, num_g, 128))
        ...
        self.pool1_r = TopKPooling(128, ratio=0.8)
        self.pool2_r = TopKPooling(128, ratio=0.8)
        self.pool3_r = TopKPooling(128, ratio=0.8)
        self.pool4_r = TopKPooling(128, ratio=0.8)
        ...
        self.meta_p4 = gnn.MetaLayer(EdgeModel(128, 512, 128),
                                     NodeModel(128, 128, 128),
                                     GlobalModel(128, 128, 256))
        self.pool1_p = TopKPooling(128, ratio=0.8)
        self.pool2_p = TopKPooling(128, ratio=0.8)
        self.pool3_p = TopKPooling(128, ratio=0.8)
        self.pool4_p = TopKPooling(128, ratio=0.8)
    ...
    def forward(self, r_batch_data, p_batch_data):
        ...
        rx, rc, re, rb, _, _ = self.pool1_r(rx, rc, re, rb)

        rx, re, rg = self.meta_r2(rx, rc, re, rg, rb)
        rx, rc, re, rb, _, _ = self.pool2_r(rx, rc, re, rb)

        rx, re, rg = self.meta_r3(rx, rc, re, rg, rb)
        rx, rc, re, rb, _, _ = self.pool3_r(rx, rc, re, rb)

        rx, re, rg = self.meta_r4(rx, rc, re, rg, rb)
        rx, rc, re, rb, _, _ = self.pool4_r(rx, rc, re, rb)

        # ================

        px, pe, pg = self.meta_p1(px, pc, pe, pg, pb)
        px, pc, pe, pb, _, _ = self.pool1_p(px, pc, pe, pb)

        px, pe, pg = self.meta_p2(px, pc, pe, pg, pb)
        px, pc, pe, pb, _, _ = self.pool2_p(px, pc, pe, pb)

        px, pe, pg = self.meta_p3(px, pc, pe, pg, pb)
        px, pc, pe, pb, _, _ = self.pool3_p(px, pc, pe, pb)

        px, pe, pg = self.meta_p4(px, pc, pe, pg, pb)
        px, pc, pe, pb, _, _ = self.pool4_p(px, pc, pe, pb)
